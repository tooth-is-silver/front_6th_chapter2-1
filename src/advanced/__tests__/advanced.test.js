...
describe("advanced 장바구니 테스트", () => {
  ...
  describe("1. 컴포넌트 렌더링", () => {
    ...
    it("초기 상태에서 5개의 상품이 선택 가능해야 함", () => {
      ...
      expect(container.textContent).toContain("상품1");
      expect(container.textContent).toContain("상품2");
      expect(container.textContent).toContain("상품3");
      expect(container.textContent).toContain("상품4");
      expect(container.textContent).toContain("상품5");
    });
  });

  describe("2. 상품 정보 및 재고 관리", () => {
    it("초기 재고가 0인 상품4는 품절 표시되어야 함", () => {
      const options = container.querySelectorAll('option');
      const product4Option = Array.from(options).find(option => option.textContent.includes('상품4'));
  ...
  describe("3. 장바구니 기능", () => {
    ...
    it("장바구니에서 상품 수량을 증가/감소할 수 있어야 함", async () => {
      ...
      // 수량 증가
      const increaseButton = container.querySelector('button[class*="quantity-change"]:last-of-type');
      if (increaseButton) {
      ...
      // 수량 감소
      const decreaseButton = container.querySelector('button[class*="quantity-change"]:first-of-type');
      if (decreaseButton) {
    ...
    it("장바구니에서 상품을 제거할 수 있어야 함", async () => {
      ...
      const removeButton = container.querySelector('button[class*="remove-item"]');
      if (removeButton) {
        removeButton.click();
        await waitForUpdate();
  ...
  describe("6. 주문 요약", () => {
    ...
    it("무료배송 안내가 표시되어야 함", () => {
      ...
      expect(container.textContent).toContain("Earn loyalty points with purchase");
    });
  });
  ...
  describe("7. 모달 기능", () => {
    it("이용 안내 모달 요소가 존재해야 함", () => {
      // 모달 토글 버튼 확인
      const modalButton = container.querySelector('button[class*="fixed top-4 right-4"]');
